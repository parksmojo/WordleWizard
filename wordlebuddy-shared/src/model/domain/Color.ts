...
export class Colors {
  ...
  private static readonly dataOf: { [key: string]: Color } = {
    [this.black]: {
      hex: "#3a3a3c",
      next: this.yellow,
      filterMethod: (word: string, char: string, pos: number) => !word.includes(char),
    },
    [this.yellow]: {
      hex: "#b59f3b",
      next: this.green,
      filterMethod: (word: string, char: string, pos: number) => word.includes(char) && word[pos] !== char,
    },
    [this.green]: {
      hex: "#538d4e",
      next: this.black,
      filterMethod: (word: string, char: string, pos: number) => word[pos] === char,
    },
    [this.orange]: {
      hex: "#ff8c00",
      next: this.orange,
